...
if [ "$#" -eq 0 ]; then
    APP_DIR="$(cd "$(dirname "$0")/.." >/dev/null 2>&1; pwd -P)/packages/zuraffa_dashboard"
...
for pkg in "zuraffa_dashboard" "zuraffa_dashboard_platform_interface" "zuraffa_dashboard_android" "zuraffa_dashboard_ios" "zuraffa_dashboard_macos"; do
    pubspec="packages/$pkg/pubspec.yaml"
    sed -i '' -E "s/^version: .*/version: $VERSION/" "$pubspec"
    sed -i '' -E "/^( *zuraffa_dashboard: \^)/s|.*|  zuraffa_dashboard: ^$VERSION|" "$pubspec"
